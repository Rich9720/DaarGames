...
if (isset($_SESSION['idCliente'])) {
    if(isset($_REQUEST['guardar'])){
        ...
        $queryCli="UPDATE clientes set nombre_completo='$nombreCli',correo='$emailCli',direccion='$direccionCli' where idClient='".$_SESSION['idCliente']."' ";
        $resCli=mysqli_query($con,$queryCli);

        $nombreRec=$_REQUEST['nombreRec']??'';
        $emailRec=$_REQUEST['emailRec']??'';
        $direccionRec=$_REQUEST['direccionRec']??'';
        $queryRec="INSERT INTO recibe_producto(nombre,email,direccion,idClient) VALUES ('$nombreRec','$emailRec','$direccionRec','".$_SESSION['idCliente']."')
        ON DUPLICATE KEY UPDATE
        nombre='$nombreRec',email='$emailRec',direccion='$direccionRec'; ";
        $resRec=mysqli_query($con,$queryRec);
        if($resCli && $resRec){
            echo '<meta http-equiv="refresh" content="0; url=catalogoindex.php?modulo=pasarela" />';
        }
        else{
        ?>
    ...
    $queryCli="SELECT nombre_completo,correo,direccion from clientes where idClient='".$_SESSION['idCliente']."';";
    $resCli=mysqli_query($con,$queryCli);
    $rowCli=mysqli_fetch_assoc($resCli);

    $queryRec="SELECT nombre,email,direccion from recibe_producto where idClient='".$_SESSION['idCliente']."';";
    $resRec=mysqli_query($con,$queryRec);
...
?>
    <form method="post">
            ...
            <div class="row">
                <div class="col-6">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="nombreCli" id="nombreCli" class="form-control" value="<?php echo $rowCli['nombre_completo'] ?>">
                    </div>
                    <div class="form-group">
        ...
        <a class="btn btn-warning" href="catalogoindex.php?modulo=carrito" role="button">Regresar al carrito</a>
